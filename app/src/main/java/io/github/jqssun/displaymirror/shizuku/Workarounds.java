...
public final class Workarounds {
  ...
  public static void apply() {
    if (Build.VERSION.SDK_INT >= AndroidVersions.API_31_ANDROID_12) {
      // on some Samsung devices, DisplayManagerGlobal.getDisplayInfoLocked() calls
      // ActivityThread.currentActivityThread().getConfiguration(),
      // which requires a non-null ConfigurationController.
      // configurationController was introduced in Android 12, so do not attempt to set it on lower
      // versions.
      // <https://github.com/Genymobile/scrcpy/issues/4467>
      // must be called before fillAppContext() because it is necessary to get a valid system
      // context.
  ...
  private static void fillConfigurationController() {
    try {
      ...
      Object configurationController =
          configurationControllerConstructor.newInstance(ACTIVITY_THREAD);
