...
public class ProjectViaDisplaylink implements Job {
  ...
  public void start() throws YieldException {
    ...
    if (!ApkImporter.areLibsImported(context)) {
      State.showErrorStatus(
          "DisplayLink libraries not imported. Please import the DisplayLink APK first.");
      return;
    }
    ...
    if (_requestMediaProjectionPermission(context, displaylinkState)) {
      displaylinkState.nativeDriver.setMode(
          displaylinkState.encoderId,
          new DisplayMode(
  ...
  private boolean _requestUsbPermission(Context context, UsbManager usbManager, UsbDevice device)
  ...
  private void _openUsbConnection(
      Context context, UsbManager usbManager, DisplaylinkState displaylinkState) {
    if (displaylinkState.displaylinkDevice2 == null) {
      ...
      if (displaylinkState.displaylinkConnection2 == null
          || displaylinkState.usbConnection == null
          || displaylinkState.usbConnection.getRawDescriptors() == null) {
        if (displaylinkState.usbConnection != null) {
  ...
  private boolean _requestDevice2UsbPermission(
      Context context, UsbManager usbManager, DisplaylinkState displaylinkState)
      throws YieldException {
    if (displaylinkState.displaylinkDevice2 == null) {
  ...
  private boolean _initializeNativeDriver(Context context, DisplaylinkState displaylinkState)
    ...
    if (displaylinkState.nativeDriver == null) {
      ...
      displaylinkState.nativeDriver.destroy();
      int resultCode =
          displaylinkState.nativeDriver.create(
              displaylinkState.nativeDriverListener, context.getFilesDir().toString(), true);
      if (resultCode != 0) {
      ...
      if (displaylinkState.displaylinkDevice2 != null) {
        displaylinkState.nativeDriver.usbDeviceDetached(
            displaylinkState.displaylinkDevice2.getDeviceName());
      }
      resultCode =
          displaylinkState.nativeDriver.usbDeviceAttached(
              deviceName,
              displaylinkState.usbConnection.getFileDescriptor(),
              displaylinkState.usbConnection.getRawDescriptors(),
              displaylinkState.usbConnection.getRawDescriptors().length);
      if (resultCode != 0) {
      ...
      if (displaylinkState.displaylinkDevice2 != null) {
        resultCode =
            displaylinkState.nativeDriver.usbDeviceAttached(
                displaylinkState.displaylinkDevice2.getDeviceName(),
                displaylinkState.displaylinkConnection2.getFileDescriptor(),
                displaylinkState.displaylinkConnection2.getRawDescriptors(),
                displaylinkState.displaylinkConnection2.getRawDescriptors().length);
        if (resultCode != 0) {
