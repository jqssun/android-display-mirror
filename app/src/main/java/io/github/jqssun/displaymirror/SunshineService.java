...
public class SunshineService extends Service {
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
            ...
            () -> {
              try {
                SunshineServer.setFileStatePath(
                    SunshineService.this.getFilesDir().getAbsolutePath() + "/sunshine_state.json");
    ...
    registerReceiver(
        usbPermissionReceiver, permissionFilter, null, null, Context.RECEIVER_EXPORTED);
  ...
  private void createNotificationChannel() {
    NotificationChannel serviceChannel =
        new NotificationChannel(
            CHANNEL_ID, "Sunshine Service Channel", NotificationManager.IMPORTANCE_LOW);
    ...
    try {
      ...
      String certPem =
          "-----BEGIN CERTIFICATE-----\n"
              ...
              + "-----END CERTIFICATE-----\n";
      // getEncoded() returns PKCS#8; OpenSSL expects PKCS#1 for "RSA PRIVATE KEY"
